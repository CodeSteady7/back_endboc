	...
	img: async (req, res) => {
		{
			try {
				...
				console.log("🚀 ~ file: report.js ~ line 49 ~ img: ~ req.files", req.files)
				const tempBody = req.body
				console.log("🚀 ~ file: report.js ~ line 50 ~ img: ~ req.body", req.body)
				// const targetPath = path.join(__dirname, "./uploads/image.png")

				if (req.file) {
					let tmp_path = req.file.path
					let originaExt =
						req.file.mimetype.split(".")[req.file.originalname.split(".").length - 1]
