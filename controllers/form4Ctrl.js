...
const form4Ctrl = {
	...
	updateForm04: async (req, res, next) => {
		try {
			...
			const gettbl_lubeoil_bearingtemperature = await tbl_lubeoil_bearingtemperature.update(
				{
					value_lubeoil_bearingtemperature: value_lubeoil_bearingtemperature,
					kode_jam: kode_jam,
					name_table: 'Lube Oil Bearing Temperature',
	...
	getForm04params: async (req, res) => {
		try {
			const tbl_lubeoil_bearingtemperatures = await tbl_lubeoil_bearingtemperature.findAll({
				where: { id: req.params.id },
			});
